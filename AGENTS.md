...
└── cloudfunctions/
    ...
    └── checkImage/     图片内容安全检测（UGC 合规，imgSecCheck）
```
...
### 5.6 用户上传图片必须过内容安全检测（审核要求）

- 微信审核要求：所有 UGC（用户上传内容）场景必须接入 `security.imgSecCheck`。
- 本项目用户可上传图片的场景：登录头像、添加/编辑菜品图。
- 实现：`checkImage` 云函数封装 `imgSecCheck`，前端上传图片后先调它检测。
- 检测不通过 → 删除已上传的云存储图 + 弹窗提示用户更换。
- 检测服务异常 → 保守放行（避免阻塞正常用户），但需在审核说明里注明已接入。

### 5.7 登录页必须有「跳过」出口（审核要求）

- 微信审核要求：登录环节必须提供显著的取消/拒绝登录按钮，不得强制登录才能体验。
- 登录页底部有「跳过，先逛逛」按钮，点了 reLaunch 回首页。
- 配合「按需登录」架构：首页是启动页可免登录浏览，登录只在进订单页时触发。
...
| `checkImage` | 图片内容安全检测 | 所有人 | 封装 imgSecCheck；检测异常时保守放行 |
